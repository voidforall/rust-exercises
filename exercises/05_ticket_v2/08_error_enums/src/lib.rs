...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(TicketNewError::InvalidTitle { reason }) => panic!("{}", reason),
        Err(TicketNewError::InvalidDescription { reason: _ }) => {
            Ticket::new(title, "Description not provided".to_string(), status)
                .expect("Default description should always be valid")
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::InvalidTitle{reason: "Title cannot be empty".to_string()});
        }
        if title.len() > 50 {
            return Err(TicketNewError::InvalidTitle{reason: "Title cannot be longer than 50 bytes".to_string()});
        }
        if description.is_empty() {
            return Err(TicketNewError::InvalidDescription{reason: "Description cannot be empty".to_string()});
        }
        if description.len() > 500 {
            return Err(TicketNewError::InvalidDescription{reason: "Description cannot be longer than 500 bytes".to_string()});
        }
